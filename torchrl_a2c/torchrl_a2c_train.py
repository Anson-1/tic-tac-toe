"""
TorchRL A2C training for Super Tic-Tac-Toe.

Advantage Actor-Critic: on-policy, no clipping (simpler than PPO).
Uses GAE for advantage estimation and a shared backbone for actor+critic.
"""
...
def collect_rollout(
    ...
    """Collect episodes with batched forward passes. Returns (TensorDict, stats)."""
    envs = [SuperTicTacToeEnv(success_rate=success_rate) for _ in range(n_episodes)]
    states = [env.reset() for env in envs]
    done_flags = [False] * n_episodes

    observations, action_masks, actions_list = [], [], []
    log_probs_list = []
...
def train(
    ...
    for update in range(1, num_updates + 1):
        ...
        # A2C update (single pass over all data — no mini-batching, on-policy)
        ...
        actor_loss = loss_td["loss_objective"].item()
        critic_loss = loss_td["loss_critic"].item()
    ...
    axes[0].set_ylim(0, 1)
    axes[0].legend()
    axes[0].set_title("TorchRL A2C Training — Super Tic-Tac-Toe")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TorchRL A2C for Super Tic-Tac-Toe")
